...
public class WaterWaveView extends View {
	...
	private void init(Context context) {
		...
		mWaveColor = context.getResources().getColor(R.color.progress);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(width / 2, width / 2, radius, mCirclePaint);
		...
		if ((!mStarted) || (width == 0) || (height == 0)) {
			...
			RectF oval = new RectF(
					width / 4 + mRingSTROKEWidth / 2 - width / 4, height / 2
							- height / 2 / 2 - 80, width - 8, height - 5);// 设置个新的长方形，扫描测量
		...
		RectF oval = new RectF(width / 4 + mRingSTROKEWidth / 2 - width / 4,
				height / 2 - height / 2 / 2 - 80, width - 8, height - 5);// 设置个新的长方形，扫描测量
		canvas.drawArc(oval, 0, 180, true, mWavePaint);
		...
		mPath.reset();
		int startX = width / 4 + mRingSTROKEWidth / 2 - width / 4;
		// 波浪效果
		while (startX < width - 6) {// width / 4 - mRingSTROKEWidth / 2 + width
									// /4
